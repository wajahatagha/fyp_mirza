...
import { userLoggedIn } from '../../Redux/reducers/userReducers';

const SignIn = () => {
  const [formData, setFormData] = useState({ emailAddress: '', password: '' });
  const dispatch = useDispatch();
  const { emailAddress, password, name } = formData;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (from === 'adminLogin') {
        ...
        res = await loginApi({ emailAddress, password, from });
      }
      console.log(res);

      if (res.error) {
        setFormData({ emailAddress: '', name: '', password: '' });
